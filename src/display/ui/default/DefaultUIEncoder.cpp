...
void DefaultUI::updateEncoderControls() {
    ...
    case EncoderAction::Profile: {
        if (controller->isActive() || controller->getMode() != MODE_BREW || !profileLoaded) {
            break;
        }
        String id;
        {
            ...
            if (std::find(ids.begin(), ids.end(), selected) == ids.end()) {
                ids.insert(ids.begin(), selected);
            }
            ...
            int index = 0;
            for (int i = 0; i < count; ++i) if (ids[i] == selected) {
                index = i;
            }
    ...
    case EncoderAction::Temperature:
        if ((page == EncoderPage::Steam && controller->getMode() != MODE_STEAM) ||
            (page == EncoderPage::Water && controller->getMode() != MODE_WATER)) {
            break;
        }
