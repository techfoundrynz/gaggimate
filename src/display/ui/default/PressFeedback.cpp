...
void styleControl(lv_obj_t *obj) {
    if (lv_obj_get_event_user_data(obj, controlMarker)) {
        return;
    }
...
void installPressFeedback(lv_obj_t *screen) {
    if (!screen || lv_obj_get_event_user_data(screen, installedMarker)) {
        return;
    }
    initStyles();
...
void setPressFeedback(lv_obj_t *control, bool pressed) {
    ...
    lv_anim_del(control, pulseTick); // a held press supersedes any running pulse
    if (!pressed) {
        lv_obj_clear_state(control, LV_STATE_USER_1);
        return;
    }
    if (lv_obj_has_flag(control, LV_OBJ_FLAG_HIDDEN) || lv_obj_has_state(control, LV_STATE_DISABLED)) {
        return;
    }
    initStyles();
...
void pulsePressFeedback(lv_obj_t *control) {
    if (!control || lv_obj_has_flag(control, LV_OBJ_FLAG_HIDDEN) || lv_obj_has_state(control, LV_STATE_DISABLED)) {
        return;
    }
    initStyles();
    styleControl(control);
