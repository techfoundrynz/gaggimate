  ...
  public:
    ...
    EncoderPress() {
        ...
        handler.setCallback([this](uint8_t, ButtonHandler::Event action) {
            if (action == ButtonHandler::Event::LONG_PRESS) event = Event::LongPress;
            else if (action == ButtonHandler::Event::CLICK && !cancelled) event = Event::Click;
        });
    }
    ...
    Event update(bool pressed, uint32_t now, int screen) {
        event = Event::None;
        if (held && screen != initialScreen) cancelled = true;
